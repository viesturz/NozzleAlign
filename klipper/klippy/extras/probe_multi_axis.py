...
class PrinterProbeMultiAxis:
    def __init__(self, config, mcu_probe_x, mcu_probe_y, mcu_probe_z):
        ...
        self.gcode.register_command('PROBE_MULTI_AXIS', self.cmd_PROBE_MULTI_AXIS,
                                    desc=self.cmd_PROBE_MULTI_AXIS_help)
        self.gcode.register_command('QUERY_PROBE_MULTI_AXIS',
    ...
    cmd_PROBE_MULTI_AXIS_help = "Probe towards specified direction"
    def cmd_PROBE_MULTI_AXIS(self, gcmd):
        direction = gcmd.get("DIRECTION").lower()
        pos = self.run_probe(direction, gcmd)
        gcmd.respond_info(
            "Result is x,y,z=%.6f,%.6f,%.6f" % (pos[0], pos[1], pos[2]))
...
def load_config(config):
    return PrinterProbeMultiAxis(config, ProbeEndstopWrapper(config,'x'),
